...
Mit dieser Definition von Toleranz kann nun die Definition der Konsistenz einer Wissensbasis definiert werden (siehe \cite{goldszmidt96}).
\begin{theorem}[Konsistenz einer Wissensbasis]
\label{def:konsistenz} todo: link this in code part
...
\end{theorem}
Aus dieser Defintion entsteht dann der folgenden Algorithmus zum Testen einer Wissensbasis auf Konsistentz, ebenfalls entnommen aus \cite{goldszmidt96}. \\

Input: A knowledge base $\mathcal{R}$ \\
Output: An ordered partition $\mathcal{R}$ is consistent \\
...
todo: ref auf theorie besser machen. link \autoref{def:konsistenz} oder sogar den algo falls der dann dort ist? \\
