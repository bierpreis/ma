...
todo: satz, dass hier kein threading stattfindet \\
...
Als Input dafür wird die Liste an WorldsLists aus \autoref{possible-worlds} mit ihren 255 Elementen verwendet. Der Algorithmus dazu ist in \autoref{create-conditionals} zu sehen. \\todo:algo beschreiben. Der Output der Methode ist eine Liste aller möglichen Konditionale in Normalform basierend auf der Liste der WorldsLists.
\begin{figure}
\begin{lstlisting}
    private List<WConditional> createBasicConditionalList(List<WorldsList> worldsList) {
        ...
        for (WorldsList currentWorld : worldsList) {
            ...
            for (WorldsList currentSubSworld : allSubSetsOfCurrentWorld) {
                //only add real subsets not equal sets //todo: shi t comment
...
\end{figure} 
\\
todo: was zur sortierung? oder das sortieren raus, aber schadet das vlt?
...
eventuell auch als eigener punkt damit das hier nicht zu viel wird. screenshots und so.
