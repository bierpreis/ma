...
Dabei sind zwei grundsätzliche Vorgehensweisen vorstellbar: Zum ersten der Ansatz, der im Folgenden als \textit{nativer} Ansatz benannt wird: Die Liste der möglichen Welten wird einfach als Disjunktion von Konjunktionen aufgefasst. Als Beispiel, unter Signatur $\Sigma=\{ab\}$ sei Liste Möglicher Welten $ab, !ab$ gegeben. Dann ist der aussagenlogische Term dafür $(a \wedge b) \vee (\overline{a}\wedge b)$. Die zweite Variante für die Übersetzung ist, den kürzesten möglichen Term zu suchen. Im genannten Beispiel ist das $a$.
\\
...
Die GUI bietet im Optionenfeld an, einige Einstellungen für den HddPairBuffer zu treffen: \textit{Delete temporary files} ist standardmäßig gesetzt und sorgt dafür, dass die nicht mehr benötigten Daten von der Festplatte automatisch gelöscht werden. Wird die Option deaktiviert, werden die Dateien nicht mehr automatisch gelöscht und können vom Benutzer betrachtet werden. Des Weiteren kann die Menge der Pairs eingestellt werden, welche zusammen in eine Datei geschrieben werden können. Voreingestellt sind hier 80.000. In dem Zusammenhang kann in den Optionen die Länge der Dateinamen eingestellt werden. Das hat folgenden Hintergrund: Wenn die Dateinamen einfach ohne \textit{leading zeroes} hochgezählt würden, bringt das die Ordnung beim Lesen der Dateien in der nächsten Iteration durcheinander. Dann wäre die Reihenfolge beim Lesen beispielsweise dann 1, 10, 11, ..., \textbf{2}, 21, 22 ... . Dadurch käme die Ordnung der Paare völlig durcheinander. Das wird vermieden, in dem die Dateinamen \textit{leading zeores} bekommen, deren Anzahl kann hier vom Benutzer eingestellt werden. Falls zu wenig leading zeoroes eingestellt werden, als benötigt würden, werden die Dateinamen einfach länger als vorgesehen, es entsteht zunächst kein Problem. \\\ todo: test, was mit leading zeores passiert. sind nicht wirklich nötig. trotzdem drinlassen? \\
