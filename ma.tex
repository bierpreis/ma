...
Die Methode \textit{void generateKbs()} ist im Gegensatz zum orginalen GenKb keinen Rückgabewert. Stattdessen werden die Outputs über drei BlockingQueues realisiert. Daneben kann die Methode eine InterruptedException werfen, nämlich genau dann wenn der Thread interrupted wird. Das passiert ausschließlich durch den Benutzer wenn er den Stop Button der GUI betätigt, dann wird die InterruptedException gefangen und der Thread beendet. Ansonsten wurde versucht, die Methode so nah wie möglich am Orginal zu gestalten. Damit und der Kommentierung mit den Zeilennummern des Orginals sollte die Methode möglichst erklärend sein. \\
...
todo: startIteration ist auch line 7! Diese Methodenaufrufe steuern die Iterationswechsel für die zwei Datenstrukturen \textit{l} und \textit{kbCreator}. Sie starten neue Threads und sorgen beim beenden von Iterationen dafür, dass auch alle Elemente einer Iteration fertig bearbeitet werden, bis die nächste Iteration beginnen kann. Dadurch wird sichergestellt, dass die Iterationswechsel sauber ablaufen und keine Daten verloren gehen. \\
