...
Die Datenstruktur, welche die im Abschnitt zuvor beschriebenen Paare aus Wissensbasis und Kandidaten verwaltet, wird im Folgenden als  \textit{PairBuffer} bezeichnet. Der PairBuffer ist damit das Element, welches in GenKb mit der Variable groß $L$ bezeichnet wird. Dabei sind im Programm zwei verschieden Varianten implementiert: Eine Option legt die Daten im Arbeitsspeicher ab, während die andere Option als Speicher die Festplatte nutzt. Der Nutzer kann in der grafischen Oberfläche in den Optionen unter "Buffering" \space wählen, welche der beiden Varianten verwendet werden soll.
...
\textit{AbstractPairBuffer} ist die Klasse, von welcher beiden Varianten der Speicherverwaltung erben und hat dabei hauptsächlich den Charakter eine Schnittstelle. Da sich die zwei Implementierungen stark unterscheiden, stellt diese abstrakte Klasse kaum eigene Funktionalität bereit und enthält dafür diverse abstrakte Methoden. Das sind zum Einen Methoden um die Iterationen zu verwalten und zum Anderen um der grafischen Oberfläche Informationen bereit zu stellen. Ansonsten ist noch die Methode \textit{hasMoreElements(int k)} zu erwähnen, mit der GenKB prüfen kann, ob eine Iteration $k$ mehr Elemente zum bearbeiten besitzt, was der GenKB Zeile 8 entspricht. Deren genaue Funktion ist Abhängig von der Variante der Implementierung.\\
Die Klasse AbstractPairBuffer besitzt zwei Instanzvariablen, welche allerdings erst in den beiden Implementierungen konkret initialisiert werden. Es handelt sich um zwei \textit{BlockingQueues} zur Verwaltung der Pairs. Blockingqueues deshalb, weil sie erstens threadsicher sind und zweitens das im Anschluss beschriebene Blocking unterstützen. Die erste Queue heißt \textit{lastIterationQueue} und stellt  die Paare zur Verarbeitung in einer Iteration $k$ bereit. Die zweite heißt \textit{nextIterationQueue} und nimmt während einer Iteration $k$ die für die nächste Iteration $k+1$ erstellen Paare auf und übernimmt deren Weiterverarbeitung.  Um diese Weiterverarbeitung kümmert sich dann jeweils ein eigener Thread innerhalb des PairBuffers. Beide Queues werden als Objekte der Klasse \textit{ArrayBlockingQueue} mit fester Größe initialisiert, diese Implementierung hat sich für die Aufgabe nach einigen Tests als gute Wahl erwiesen. \\
Durch die feste Größe der Queue ist sichergestellt, dass die Queues nicht endlos groß Wachsen können. In Java sind BlockingQueues gut geeignet für Consumer-Producer Patterns wie hier. Wenn eine Queue voll ist, wartet ein Thread automatisch bis wieder ein Platz frei wird, bevor das nächste Element abgelegt werden kann. Analog wartet ein Thread automatisch, wenn er ein Element aus der Queue entnehmen will, diese jedoch im Moment keine Elemente beinhaltet. Der Thread läuft hier dann automatisch weiter, wenn wieder ein Element bereitgestellt wurde. Dadurch sorgen die Queues dafür, dass Producer und Consumer aufeinander warten und dabei ineffizientes \textit{busy wait} vermeiden.\\
Im Falle der lastIterationQueue ist der Thread von GenKB in der Rolle des Consumers, welcher die Pairs konsumiert und weiterverarbeitet. Der Thread im PairBuffer legt die Pairs in der Queue bereit und ist damit der Producer. Bei der nextIterationQueue verhält es sich genau anders herum: GenKB produziert Pairs und legt sie in der Queue ab, während der Thread im PairBuffer der Consumer ist und die Pairs der Queue entnimmt.
Die beiden Queues werden von der Impementierung von GenKB wie folgt verwendet: Die lastIterationQueue stellt die Pairs aus Iteration $k$ bereit, damit GenKB sie nacheinander entnehmen und bearbeiten kann. Das entspricht der Zeile 8 im Algorithmus. Die nextIterationqueue nimmt die neu erstellten Paare in GenKB auf, das entspricht den Schritten in Zeile 5 und 12. Die Implementierung von GenKB greift dann also nicht auf den PairBuffer selbst zu, sondern beide Teilen sich die Queue und tauschen die Daten darüber aus.


\subsubsection{RamPairBuffer}
\label{sec:rampairbuffer}
todo: das neue zeug \\
Der RamPairBuffer besitzt eine Datenstruktur in der Form \textit{List<List<CompressedPair>>} und zwei Threads. Die Threads stellen die Verbindung zwischen der Datenstruktur und den beiden BlockingQueues dar, die von GenKB verwendet werden. \\
Ein Thread kümmert sich um die \textit{newIterationQueue}, also um die von GenKB in Zeile neu erstellten Paare als Instanzen von RealPair. Sobald die Queue Pairs enthält, entnimmt der Thread die Pairs und wandelt sie in CompressedPairs um. Dazu Wird der Konstruktor \textit{CompressedPair(RealPair)} genutzt. Die neu erstellen CompressedPairs werden dann in die Datenstruktur unter dem aktuellen Wert für $k+1$ abgelegt. Die Kompression findet in dem  dedizierten Thread statt und nicht in der Impelementierung von GenKb selbst, weil das GenKB eine Menge Arbeit abnimmt und somit deutlich beschleunigt. \\
...
Die beiden Queues werden jeweils als \textit{ArrayBlockingQueue(1000)} mit der Größe 1000 initialisiert. Die genaue größe ist nicht wirklich wichtig. Sie ist mit dem Wert groß genug, dass sie den Prozess nicht verlangsamt aber auch nicht so groß dass sie zu viel Speicher benötigen würde. \\
Die Liste in der Form \textit{List<List<CompressedPair>>} wird von einigen Methoden zum Iterationswechsel bearbeitet. Das beinhaltet natürlich das anlegen neuer Listen, aber insbesondere auch das löschen der Listen, welche nicht mehr benötigt werden. 
\\todo: prepare und finish. iteration beschreiben.
\subsubsection{HDDPairBuffer}
\label{sec:hddBuffer} 
todo: das neue zeug \\
todo: init wert für queue \\
