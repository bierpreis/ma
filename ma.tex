...
%todo: see link for this. geometry and twoside fuk up
\usepackage[twoside,left=2cm,right=2cm,top=2cm,bottom=2cm]{geometry}
%newgeometry{twoside,left=2cm,right=2cm,top=2cm,bottom=2cm}
...
\newcommand{\aref}[1]{\hyperref[#1]{Anhang~\ref{#1}}}
...
\title{Systematische Generierung konsistenter Mengen qualitativer Konditionale}
...
\def\RQEQ{{%
    \setbox0\hbox{$\prec$}%
    \rlap{\hbox to \wd0{\hss \raisebox {0.05pt}{$\ \cdot$\hss}}}\box0
}}


% rdotl eq
\newcommand\rdotleq{\mathrel{%
    \mathchoice{\RRQEQ}{\RRQEQ}{\RRQEQ}{\RRQEQ}%
}}


\def\RRQEQ{{%
    \setbox0\hbox{$\preccurlyeq$}%
    \rlap{\hbox to \wd0{\hss \raisebox {0.5pt}{$\ \cdot$\hss}}}\box0
}}

\begin{document}

\pagenumbering{gobble} %mute page numbering


\begin{titlepage}

	{\LARGE\bfseries Fernuniversität Hagen\par}
	\vspace{1cm}
	{\Large\bfseries Fakultät für Informatik\par}
	\vspace{1.5cm}
	
	{Masterarbeit\par}
	{zur Erlangung des Grades\par}
	{Master of Science Wirtschaftsinformatik\par}
	\vspace{1.5cm}
	{über das Thema}
	\vspace{1.5cm}
	
	\centering
	{\huge\bfseries Systematische Generierung konsistenter\\ Mengen qualitativer Konditionale\par}
	
	\vspace{4cm}
	\raggedright %linsbündig
	
	\begin{tabular}[]{ll}
	Eingerreicht bei: & Prof. Dr. Christoph Beierle \\
	 & \\
	Von: & Frank Leuchtmann \\
	Mart. Nr.: & 9529268\\
	Anschrift: & Herbststraße 20 \\
	& 01139 Dresden \\
	E-Mail: & frank.leuchtmann@gmx.de
	\end{tabular}
...
%noindent verhindert einrückung
\begin{abstract}\noindent
In dieser Arbeit wird ein Algorithmus zum Erstellen von konsistenten Wissensbasen über einer vorgegebenen Signatur experimentell implementiert. Für diese Aufgabe wird ein Java Programm erstellt, welches den Algorithmus umsetzt und dessen Ergebnisse persistent auf einem Datenträger speichert. Dabei zeigt sich, dass der Algorithmus wie beabsichtigt große Mengen an Wissensbasen erzeugen kann und dabei korrekt arbeitet. Dafür wird viel Speicher benötigt, daher ist der Speicher bei entsprechend langer Laufzeit auch der begrenzende Faktor für die Generierung der Wissensbasen. Für eine gegebene Signatur $\Sigma_{ab}$ mit zwei Elementen können mit dem erstellten Programm alle durch den Algorithmus generierten konsistenten und inkonsistenten Wissensbasen erstellt werden. Für eine  Signatur $\Sigma_{abc}$ mit drei Elementen ist  aufgrund der großen Menge der möglichen Wissensbasen nur in Ansätzen möglich.
\end{abstract}
...
Mit qualitativen Konditionalen in der Form \glqq Wenn A, dann normalerweise B\grqq  \space kann Wissen unter Unsicherheit repräsentiert werden. Eine  Menge solcher Konditionale lässt zu einer Wissensbasis zusammenfassen. Die Programmbibliothek InfOCF (siehe dazu \cite{kutsch19})  bietet verschiedene Funktionen zur Arbeit mit Konditionalen und Wissensbasen. Um dieses System experimentell zu evaluieren werden größere Mengen von Wissensbasen benötigt. \textit{GenKB} (siehe \cite{beierle19}) ist ein Algorithmus zur systematischen Generierung derartiger Wissensbasen. Das Ziel dieser Arbeit ist es, diesen Algorithmus in ein Programm zum implementieren und die dabei erstellten Wissensbasen persistent zu speichern.\\
Für die Bearbeitung der Aufgabe wird eine Java Programm erstellt und dessen Funktionsweise hier beschrieben. Das Programm ist so aufgebaut, dass es Wissensbasen sowohl basierend auf der Signatur  $\Sigma = \{a,b,c\}$ aus der Aufgabenstellung, als auch für eine reduzierte Version mit $\Sigma = \{a,b\}$ generieren kann. Diese reduzierte Version dient als Hilfestellung zur Bearbeitung der Aufgabe. Zusätzlich zeigt sich dadurch die Problemlösung an manchen stellen etwas besser nachvollziehbar. \\
...
\section{Grundlagen}
In diesem Abschnitt werden kurz die Grundlagen aufgeführt, auf denen der Hauptteil dieser Arbeit basiert. Die Inhalte und Darstellungen sind dabei aus den Kapiteln über Grundlagen aus \cite{beierle19} und \cite{beierle17} übernommen.
\subsection{Notation}
...
Im weiteren Verlauf wird oft für die Darstellung von Konditionalen eine Notation mittels möglicher Welten verwendet. Dazu wird anstatt von Aussagenvariablen die Menge der möglichen Welten aufgeführt, in denen die Aussage gilt. Für eine Aussage $F$ also wird die Form $\Omega_F = \{\omega | \omega \models F \}$ verwendet. Ein vollständiges Beispiel für die Aussage $A\vee\overline{B}\vee\overline{C}$ sieht dann wie folgt aus: $\Omega_{a\vee\overline{b}\vee \overline{c}} = \{abc,ab\overline{c},a\overline{b}c,a\overline{b}\overline{c},\overline{a}b\overline{c},\overline{a}\overline{b}c,\overline{a}\overline{b}\overline{c}\}$. \\
...
Das Ziel der Normalform ist die klare Definition einer vollständigen und minimalen  Menge an nichttrivialen Konditionalen über einer Signatur $\Sigma$. Die Konditionale sollen dabei untereinander nicht äquivalent sein. Diese Normalform wird als $NFC(\Sigma)$ (Normal Form Conditionals) bezeichnet.
...
Rangfunktion angewandt für Aussagen:
\[
 \kappa(A)=\begin{cases}
            ...
            \end{cases}
\]
Rangfunktion angewandt für Konditionale:
\[
...
\begin{theorem}[Isomorphie]\ \\(entnommen aus \cite{beierle19}) \ \\
...
Das Programm ist leicht zu benutzen, es bringt für alle Funktionen eine grafische Benutzeroberfläche mit. Zum Starten führt man einfach die Datei \textit {Conditionals.jar} aus. Beim Starten des Programms ohne explizite Optionen für die JVM bekommt es automatisch einen vom Arbeitsspeicher des PCs abhängigen maximal verwendbaren Arbeitsspeicher zugewiesen. Wenn mit dem Programm keine sehr speicherintensiven Aktionen ausgeführt werden sollen ist das ausreichend. Es ist ansonsten auch möglich, dem Programm eine benutzerdefinierte maximale Menge an Arbeitsspeicher zuzuweisen. Dazu startet man das Programm in der Datei \textit{Conditionals.jar} mit folgendem Konsolenbefehl: 
...
Die Klasse \textit{WConditional} (W wie worlds) dient dazu, mit zwei der im Abschnitt zuvor beschriebenen WorldsList Objekten Konditionale zu repräsentieren. Der Fokus der Klasse liegt dabei auf der Generierung, Ordnung und Betrachtung von Äquivalenz der Konditionale. Ein vereinfachtes Diagramm der Klasse ist in \autoref{pic:wconditional} zu sehen. Attribute der Klasse sind zum einen Antecedent und Consequence in Form eines WordsList Objekts. Weiterhin die Referenz \textit{counterConditional} ebenfalls als Objekt der Klasse WConditional und einer Liste von äquivalenten Konditionalen als Objekte von WConditional. Außerdem ist die Nummer des Konditionals gemäß der Ordnung $\rdotl$ nach Defintion \ref{def:sortierung-nfc} als \textit{int} Variable enthalten. Die Klasse WConditional ist ebenfalls sortierbar mit dem Interface \textit{Comparable}, die Implementierung der \textit{compareTo(Object 0)} Methode ist in \autoref{code:compare-wconditonal} gezeigt. Die Implementierung entspricht damit genau der Sortierung nach $\overset{\mathrm{c}}{\dotll}$ wie in \autoref{eq:conditional-order} beschrieben.



\begin{figure}
\includegraphics[width=0.55\linewidth]{bilder/wconditional.png}
\caption{Klasse \textit{WConditional}}
...
Die grundlegende Klasse der Aussagenlogik ist die abstrakte Klasse AbstractFormula, welche in \autoref{pic:abstractformula} als Diagramm dargestellt ist. Alle Aussagen basieren auf dieser Klasse und entsprechend erben alle Aussagen repräsentierenden Objekte von dieser. Sie stellt so die Schnittstelle für alle Aussagen dar und bietet auch selbst einige Funktionen. Die einzige Variable ist die gewählte Signatur \textit{signature} in Form einer Instanz von AbstractSignature als statisches Objekt für alle Instanzen von erbenden Objekten. Weiterhin besitzt die Klasse 6 Methoden, von denen 4 konkret implementiert und 2 abstrakt sind. Implementiert ist zum einen die Methode \textit{and(AbstractFormula otherFormula)}, sie gibt einfach ein Objekt \textit{Conjunction} zurück welches eine Konjunktion aus dem aktuellen (this) Objekt und dem Übergebenen ist. Überschrieben wird diese Methode nur in der Klasse Conjunction. Analog dazu verhält sich die Methode \textit{or(AbstractFormula otherFormua)}, welche ein Objekt der Klasse \textit{Disjunction} zurückgibt. Diese Methode wird ausschließlich von der Klasse \textit{Disjunction} überschrieben. Die dritte konkret implementierte Methode ist die Negation, aufrufbar mit \textit{neg()} ohne Parameter. Diese Methode gibt eine Negation in Form von \textit{new Negation(this)} zurück. Überschrieben wird diese Methode nur im der Klasse \textit{Negation} selbst. Die letzte konkrete Methode der Klasse \textit{AbstractFormula} ist die Methode \textit{equals(Object o)}, die in \autoref{code:equals-formula} gezeigt ist. Sie ist als \textit{final} deklariert und wird demzufolge von keiner erbenden Klasse überschrieben. Wie im Code zu sehen, überprüft die Methode zuerst, ob das übergebene Objekt eine Instanz der Klasse \textit{AbstractFormula} darstellt. Dann wird für alle möglichen Welten der Signatur geprüft, ob die Evaluierung der Welt des übergebenen Objekts das gleiche Ergebnis zurückgibt wie die Evaluierung des aktuellen Objekts (this). Ist das einmal nicht der Fall, wird sofort \textit{false} zurückgegeben. Wenn die Evaluierung für alle möglichen Welten das gleiche Ergebnis zurückgibt, sind die beiden Objekte gleich im Sinne der \textit{equals(Object o)} Methode. Weiterhin existieren die abstrakten Methoden \textit{evaluate(World world)} und \textit{toString()}, die jeweils von allen erbenden Klassen überschrieben werden.


\begin{figure}
\includegraphics[width=0.55\linewidth]{bilder/AbstractFormula.png}
\caption{Klasse \textit{AbstractFormula}}
...
Ursprünglich existierte auch eine Implementierung einer Wissensbasis, die anstatt einer \textit{List<PConditional>} eine \textit{List<Integer>} zur Repräsentation der Konditionale verwendet hat. Dies sollte den Verbrauch von Arbeitsspeicher reduzieren. Die Ersparnis dadurch war aber gering, dafür kostete das Verfahren mehr Rechenzeit. Deswegen wurde diese Variante wieder entfernt.
...
In der originalen Darstellung von GenKB wird diese Datenstruktur als \glqq L\grqq \space bezeichnet. Bearbeitet werden in einer Iteration $k$ jeweils $L_k$ und  $L_{k+1}$. Die Datenstruktur ist im Grunde eine Liste die für jedes Element $L_k$ wieder eine Liste mit den Datensätzen für die jeweilige Iteration beinhaltet. Diese Struktur wird hier folgendermaßen dargestellt: Einzelne Datensätze werden als Paare gespeichert, welche jeweils eine Wissensbasis und eine Menge an Kandidaten enthalten. Die Verwaltung dieser Datensätze übernimmt ein Puffer, der die Daten wahlweise entweder im Arbeitsspeicher oder auf der Festplatte speichert und rechtzeitig zur Verfügung stellt. Um die große Menge der Daten besser handhaben zu können, werden die Datensätze dabei jeweils zur Speicherung komprimiert und vor der Benutzung wieder rekonstruiert. Im nächsten Abschnitt wird die Speicherung der Paare aus Wissensbasen und Kandidaten gezeigt. Danach wird die Datenstruktur vorgestellt, welche die Menge dieser Paare verwaltet.

\subsubsection{Paare aus Wissensbasis und Kandidaten}

In diesem Abschnitt wird beschrieben, wie die Paare $\langle R, C \rangle$ aus Wissensbasis und Kandidaten als Java Objekte dargestellt werden. Im Algorithmus GenKB in \cite{beierle19} werden diese Paare in Zeile 5 initialisiert und in Zeile 8ff bearbeitet. Dargestellt wird diese Einheit mit dreieckigen Klammern in der Form $\langle R, C \rangle$, was im Folgenden bei Gelegenheit auch so beibehalten wird. Im Programm werden zwei Implementierungen zur Repräsentation dieser Paare genutzt, welche hier vorgestellt werden. Beide erben von der Klasse \textit{AbstractPair}, was die Gemeinsamkeiten vereint und die Schnittstelle der Paare bildet. Die beiden Implementierungen unterscheiden sich hauptsächlich in der Art der Speicherung der Kandidaten. Da davon sehr viele verwaltet werden müssen, machen sie den größten Teil des Speicherverbrauchs eines Paares und damit des gesamten Programms aus. Die erste Implementierung ist \textit{RealPair}, hier werden die Kandidaten als Liste von Konditionalen verwaltet. Das ist naheliegend, einfach und schnell, benötigt allerdings sehr viel Arbeitsspeicher. Die zweite Implementierung \textit{CompressedPair} komprimiert die Liste der Kandidaten und reduziert den benötigten Arbeitsspeicher, ist dafür jedoch langsamer. Im späteren Programmablauf arbeitet GenKB selbst mit RealPairs. Die Klasse CompressedPair bietet dann die Möglichkeit den Speicherbedarf zu reduzieren, wenn die Daten im Arbeitsspeicher zwischengespeichert werden sollen.
...
\caption{Klasse \textit{AbstractPair}}
...
Durch die Möglichkeit der Speicherung der Daten zwischen zwei Iterationen auf der Festplatte ist für den Programmablauf kein großer Arbeitsspeicher nötig. Der Algorithmus kann so sehr lange laufen und viele Wissensbasen erzeugen. Begrenzend könnte irgendwann der Platz auf der Festplatte werden. Aufgrund der großen Menge der möglichen Wissensbasen werden mit der Signatur $\Sigma_{abc}$ nur sehr geringe Werte für k erreicht. Bei entsprechend langer Laufzeit sollte es möglich sein die Iteration $k=2$, also die Wissensbasen mit drei Elementen, vollständig berechenbar sein. Höhere Iterationen würden wohl zu lange dauern und zu viel Speicher benötigen. Die persistente Speicherung der als Ergebnis erstellten Wissensbasen in Textdateien benötigt aufgrund ihrer Menge viel Speicherplatz. Bei entsprechend langer Laufzeit des Programms kann dieser Speicherplatz zum begrenzenden Faktor werden. Der kritische Faktor für die Geschwindigkeit des Programmablaufs ist die Geschwindigkeit des Prozessors. Teilweise kann durch Threading die Arbeit auf mehrere Kerne verteilt werden. Der Flaschenhals ist jedoch der Thread der GenKB selbst umsetzt.\\
Das ließe sich durch Multithreading lösen. Ein Ansatz für GenKB mit Multithreading wurde während der Entwicklung des beschriebenen Programms ausprobiert. Das parallele Erzeugen der Wissensbasen mit einem ExecutorService und mehreren Threads hat auch funktioniert. Im getesteten Threading wurden die Zeilen 9-12 von GenKB auf verschiedene Threads verteilt und dort bearbeitet. Gescheitert ist es jedoch am Speichern der Ergebnisse in der Reihenfolge wie sie der ursprüngliche Algorithmus vorgibt. Die Ergebnisse der parallelen Threads müssten dann nämlich wieder genau in der richtigen Reihenfolge eingesammelt werden. Mit einem von vornherein auf Multi Threading ausgerichteten Design dürfte es aber möglich sein, mehrere Prozessorkerne für den Hauptalgorithmus zu nutzen und die Geschwindigkeit dadurch zu steigern. Dazu müsste das Design des Programms jedoch grundsätzlich neu konzipiert werden.

\subsection{Programmablauf mit Signatur a,b}
Mit der Signatur $\Sigma_{ab}$ kann das Programm tatsächlich alle Wissensbasen generieren. Dabei entstehen 364.144.824 konsistente und 364.304.482 inkonsistente Wissensbasen in 25 Iterationen. Die Wissensbasen als Ergebnis im InfOCF Format benötigen dafür 257.1 GB Festplattenspeicher, der ganze Vorgang hat auf dem Test PC (aktueller Mittelklasse PC: AMD Ryzen 3600 und 32 GB RAM) 1 Stunde und 52 Minuten gedauert. Dabei wurde die Zwischenspeicherung der Daten im Arbeitsspeicher des PCs verwendet. Der JVM mussten dafür mindestens 23 GB RAM zugewiesen werden. \\
...
\subsection{Programmablauf mit Signatur a,b,c}
...
Mit der Signatur $\Sigma_{abc}$ und tatsächlichem Schreiben der Wissensbasen auf dem Datenträger fallen bei Beobachtung der grafischen Oberfläche beim Programmablauf die Zähler unter \textit{Overall Status} und \textit{ Current Iteration Status} auf. Dabei ist insbesondere der längere Zeit auf 0 stehende Zähler \textit{InconsistentKnowledgeBases} verwunderlich. Das liegt daran, dass die Wissensbasen erst dann gezählt werden, wenn sie tatsächlich geschrieben werden. Da recht wenige inkonsistente Wissensbasen entstehen dauert es recht lange bis genug Wissensbasen für die erste Datei zusammengekommen sind und der Zähler dann über den Wert von 0 springt. \\
Die Iteration für $k = 1$ ist auf dem Test PC noch problemlos ohne Zwischenspeicherung der Daten auf der Festplatte durchführbar. Die dafür benötigte Zeit betrug im Test 266 Sekunden . Als Ergebnis entstehen 7.975.405 konsistente und 210.200 inkonsistente Wissensbasen. Im Durchschnitt wurden also ca. 30.000 konsistente Wissensbasen pro Sekunde erstellt. Die gespeicherten Wissensbasen als Ergebnis benötigen dann etwa 1,1 GB Speicher, abhängig davon wie viele Wissensbasen man pro Datei schreiben lässt. \\
...
In diesem Abschnitt soll kurz die Betrachtung des erstellten Programms mit dem Programm JVisualVM gezeigt werden. JVisualVM ist ein Profiler, mit dem Java Programme während der Laufzeit näher betrachtet werden können, eine Beschreibung zum Programm befindet sich unter \cite{jvis20}. Mit diesem Tool lassen sich insbesondere Informationen zum Speicherverbrauch und zur CPU Nutzung von Java Anwendungen finden. Es lassen sich weiterhin Details zu laufenden Threads und zur Rechenzeit einzelner Methoden untersuchen. \\
Bei einer Anwendung des Profilers auf KBCreator zeigt sich wie erwartet, dass der GenKB Thread mit Abstand am meisten Rechenzeit benötigt. Dieser Thread ist wie erwartet der Flaschenhals im Programm und hier müsste man ansetzen, wenn der Ablauf beschleunigt werden soll. Im GenKB Thread selbst ist wie zu erwarten  die \textit{isConsistentWith(Conditional)} Methode der Klasse KnowledgeBase die Stelle, an der am meisten Rechenzeit verbraucht wird. \\
...
Alle Abbildungen in diesem Dokument sind eigene Darstellungen. Die UML Diagramme wurden mit der Software \glqq Dia\grqq \space erstellt, siehe dazu \cite{dia20}. Die schematischen Darstellungen zum Überblick über das erstellte Programm sind ebenfalls mit der Software \glqq Dia\grqq \space erstellt. Screenshots  wurden mit dem Programm \glqq Xfce4-Screenshooter\grqq \space erstellt.

%this hides anhang from tocus
%\addtocontents{toc}{\protect\setcounter{tocdepth}{0}}


\section{Anhang GenKB original Version}
\label{app:genkb}

%dunno why then must be highlighted manually
\begin{lstlisting}[mathescape]
$L_1 \leftarrow \emptyset$
$k \leftarrow 1$
for $r \in cNFC(\Sigma)$ do
  $D \leftarrow \{d|d \in [r']_\simeq, r' \in cNFC(\Sigma), r' \rdotl r \}$
  $L_1 \leftarrow L_1 \cup \{\langle \{r\}, NFC(\Sigma) \setminus (\{\overline{r}\} \cup D) \rangle \} $
while $r \neq \emptyset $ do
  $L_{k+1} \leftarrow \emptyset $
  for $\langle \mathcal{R}, C \rangle \in L_k $ do
    for $r \in C $ do
      if $\mathcal{R} \cup \{r \} $ is consistent $\textbf{then}$
        $D \leftarrow \{d|d \in C, d \rdotleq r \} $
        $L_{k+1} \leftarrow L_{k+1} \cup \{ \langle \mathcal{R} \cup \{r\}, C \setminus (\{\overline{r}\} \cup D ) \rangle \}$ 
  $k \leftarrow k + 1$
return $ \mathcal{KB}  = \{\mathcal{R} | \langle \mathcal{R}, C \rangle \in L_i, i \in \{ 1, ..., k)  \}\}$
