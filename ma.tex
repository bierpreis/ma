...
Die Liste in der Form \textit{List<List<CompressedPair>>} wird von einigen Methoden zum Iterationswechsel bearbeitet. Dazu wird beispielsweise durch die \textit{prepareIteraiton(int kk)} Methode eine neue Liste für k angelegt. Die \textit{finishIteration(int k)} Methode sort dafür, dass alle Pairs aus den Queues auch fertig bearbeitet sind, bevor die neue Iteration beginnt. Dadurch wird ausgeschlossen, dass durch den Iterationswechsel Datensätze verloren gehen können. \\
...
Die GUI bietet im Optionenfeld an, einige Einstellungen für den HddPairBuffer zu treffen: \textit{Delete temporary files} ist standardmäßig gesetzt und sorgt dafür, dass die nicht mehr benötigten Daten von der Festplatte automatisch gelöscht werden. Wird der Button deaktiviert, werden die Daten nicht mehr automatisch gelöscht und können vom Benutzer betrachtet werden. Des weiteren kann die Menge der Pairs eingestellt werden, welche zusammen in eine Datei geschrieben werden können. Voreingestellt sind hier 80.000. Des Weiteren kann in den Optionen die Länge der Dateinamen eingestellt werden. Das hat folgenden Hintergrund: Wenn die Dateinamen einfach ohne \textit{leading zeroes} hochgezählt würden, bringt das die Ordnung beim Lesen der Dateien in der nächsten Iteration durcheinander. Dann wäre die Reihenfolge beim Lesen beispielsweise dann 1, 10, 11, ..., 2, 21, 22 ... . Dadurch käme die Ordnung der Paare völlig durcheinander. Das wird vermieden, in dem die Dateinamen \textit{leading zeores} bekommen, deren Anzahl kann hier vom Benutzer eingestellt werden.\\
...
todo: einfügen:  In der GUI kann der nutzer die Option setzen, diese Daten nicht zu löschen. Dadurch können sie bei Bedarf betrachtet werde.
