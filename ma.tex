...
Die Bezeichung der möglichen Welten wurde in \autoref{ordnungsrelation} schon gezeigt, an diesem Punkt setzt die Betrachtung hier an. Wie beschrieben, lassen sich aussagenlogische Ausdrücke als Menge der damit bezeichneten Welten ausdrücken. Aus der Gesamtheit dieser Mengen ergibt sich die Menge der Ausdrücke, die in den Konditionalen als Antecedent oder Consequence eingesetzt werden kann. Für die Signatur $\Sigma = {abc}$ ergeben sich damit 255 verschiedene Mengen von Welten. Diese können mit dem NFC-Viewer mit dem Button "WORLDS" geordnet erzeugt werden. Dargestellt im Programm werden diese Mengen jeweils durch eine Instanz der Klasse WorldsList, von der ein vereinfachtes Diagramm in \ref{worldslist} gezeigt ist. Das wichtige Attribut dieser Klasse ist die Liste an möglichen Welten. Als Methoden stehen compareTo(Wordslist otherList) zur Sortierung gemäß Defintion \ref{def:ordering} und eine Methode zur Prüfung von Äquivalenz zur Verfügung. \\
