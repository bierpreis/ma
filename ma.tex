...
Durch die feste Größe ist sichergestellt, dass die Queues nicht endlos groß Wachsen können. In Java sind ArrayBlockingQueues gut geeignet für Consumer-Producer Patterns wie hier. Wenn eine Queue voll ist, wartet ein Thread automatisch bis wieder ein Platz frei wird, um es darin ablegen zu können. Analog wartet ein Thread automatisch, wenn er ein Element aus der Queue entnehmen will wenn sie leer ist. Er läuft dann weiter, wenn wieder ein Element vorhanden ist. Dadurch sorgen die Queues dafür, dass Producer und Consumer aufeinander warten aber dabei ineffizientes \textit{busy wait} vermeiden.\\
Im Falle der lastIterationQueue ist der Thread von GenKB der Consumer welcher die Pairs konsumiert und weiterverarbeitet. Der Thread im PairBuffer legt die Pairs rechtzeitig bereit und ist damit der Producer. Bei der nextIterationQueue verhält es sich genau andersrum: GenKB produziert Pairs und legt sie in der Queue ab, während der Thread im PairBuffer der Consumer ist und die Pairs der Queue entnimmt.
