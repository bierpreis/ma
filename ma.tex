...
Das Ziel dieser Arbeit ist es, möglichst viele Wissensbasen auf Basis der Signatur $\Sigma=\{a,b,c\}$ zu generieren. Da aufgrund der Menge der möglichen Konditionale das Ziel nicht auch nur Ansatzweise vollständig bearbeitet werden kann, wird immer auch die Signatur $\Sigma=\{a,b\}$ betrachtet. Mit dieser reduzierten Signatur ist es möglich, aufgrund der deutlich geringeren Menge an Konditionalen alle möglichen Wissensbasen zu generieren. Damit lässt sich prüfen, ob die grundsätzliche Vorgehensweise korrekt ist und letztendlich die Aufgabenstellung vollständig lösen könnte. \\
Dazu wird die Aufgabenstellung zunächst in zwei Teile untergliedert. Im ersten Teil werden basierend auf der Signatur die Konditionale in Normalform berechnet und somit die Menge $NFC$  berechnet. Anschließend werden die Äquivalenzklassen Untersucht und damit die Menge $cNFC$ sowie die einzelnen Äquivalenzklassen berechnet. \\
Dazu werden im ersten Teil die Konditionale in Normalform basierend auf der Klasse \textit{WConditional} (W für Worlds) bearbeitet. Diese Repräsentation spiegelt die Darstellung von Konditionalen in Normalformen in \cite{beierle19} wieder. Diese Darstellung ist außerdem dafür optimiert, die in \cite{beierle19} dargestellte Ordnung im Sinne von $\dotl$ zu repräsentieren und ermöglichst so die Ordnung der Konditionale und die Behandlung von Äquivalentzen. Diese Konditionale werden dann für den zweiten Teil in die Klasse \textit{PConditional} (P für Propositional logic) übersetzt. Diese Klasse ist dann für die Arbeit mit Aussagenlogik optimiert und ermöglicht so die Prüfung von Konsistenz und damit die Anwendung für den Algorithmus GenKB.
\section{Konditionale in Normalform}
\subsection{Die Klasse WConditional}
Hier was zur Klasse WConditional (Klassendiagramm wär nice), zu World und WorldsDifference (besonders isEquivalent()). Auch zu compare und comparable (hier zum ersten mal)
\subsection{Mögliche Welten}
Erstellung, Comparable, Auflistung für AB und Nennung ABC?
\subsection{NFC}
...
ALT:
%todo: noch erwähnen, dass nfc und cnfc nach den verschiedenen ordnungen nummeriert werden. auch nochmal durchdenken das ganze, stimmt das so wie oben beschrieben?
%todo: screenshots neu machen!!
\begin{figure}
	\centering
