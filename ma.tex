...
Die Klasse AbstractPairBuffer besitzt zwei Instanzvariablen, die auch bereits in der abstrakten Klasse initialisiert werden. Es handelt sich um zwei \textit{BlockingQueues} zur Verwaltung der Pairs. Zum einen eine \textit{lastIterationQueue}, welche die Paare der Iteration $k$ bereitstellt. Zum andern die \textit{nextIterationQueue}, welche die in der Iteration $k+1$ erstellen Paare aufnimmt und je nach Implementierung weiterverarbeitet. Um diese Weiterverarbeitung kümmert sich jeweils ein eigener Thread, der in den folgenden Implementierungen beschrieben wird. Beide Queues werden als Objekte der Klasse \textit{ArrayBlockingQueue} initialisiert. Die Klasse \textit{LinkedBlockingQueue} bietet für diese Anwendung die selbe Funktionalität und zeigt auch etwa gleiches Verhalten in der Performance, wäre also ebenfalls geeignet gewesen. \\
