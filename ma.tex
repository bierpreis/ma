...
\textit{AbstractPairBuffer} ist die abstrakte Klasse, von welcher beiden Varianten der Speicherverwaltung erbenund hat dabei hauptsächlich den Charakter eine Schnittstelle. Da sich die zwei Implementierungen stark unterscheiden, stellt diese abstrakte Klasse kaum eigene Funktionalität bereit und enthält dafür diverse abstrakte Methoden. Das sind zum Einen Methoden um die Iterationen zu verwalten und zum Anderen um der grafischen Oberfläche Informationen bereit zu stellen. \\
Die Klasse AbstractPairBuffer besitzt zwei Instanzvariablen, die die allerdings erst in den beiden Implementierungen konkret initialisiert werden. Es handelt sich um zwei \textit{BlockingQueues} zur Verwaltung der Pairs. Blockingqueues deshalb, weil sie erstens threadsicher sind und zweitens das im Anschluss beschriebene Blocking unterstützen.Zum einen eine \textit{lastIterationQueue}, welche die Paare der Iteration $k$ bereitstellt. Zum andern die \textit{nextIterationQueue}, welche die in der Iteration $k+1$ erstellen Paare aufnimmt und je nach Implementierung weiterverarbeitet. Um diese Weiterverarbeitung kümmert sich jeweils ein eigener Thread, der in den folgenden Implementierungen beschrieben wird. Beide Queues werden als Objekte der Klasse \textit{ArrayBlockingQueue} initialisiert. Die Klasse \textit{LinkedBlockingQueue} bietet für diese Anwendung die selbe Funktionalität und zeigt auch etwa gleiches Verhalten in der Performance, wäre also ebenfalls geeignet gewesen. \\
Durch die feste Größe der Queue ist sichergestellt, dass die Queues nicht endlos groß Wachsen können. In Java sind BlockingQueues gut geeignet für Consumer-Producer Patterns wie hier. Wenn eine Queue voll ist, wartet ein Thread automatisch bis wieder ein Platz frei wird, bevor das nächste Element abgelegt werden kann. Analog wartet ein Thread automatisch, wenn er ein Element aus der Queue entnehmen will, diese jedoch im Moment keine Elemente beinhaltet. Der Thread läuft erst dann automatisch weiter, wenn wieder ein Element vorhanden ist. Dadurch sorgen die Queues dafür, dass Producer und Consumer aufeinander warten und dabei ineffizientes \textit{busy wait} vermeiden.\\
