...
Die Klassen \textit{World} und \textit{Signature} hängen eng zusammen. Die Basis für alle Möglichen Welten bildet die Klasse \textit{AbstractWorld}. Diese bietet die abstrakte Methode \textit{boolean get(Var var)} mit dem Parameter \textit{Var}, einer Enum die $a$, $b$ oder $c$ annimmt und eine Variable der Signatur repräsentiert. Implementiert wird die Klasse AbstractWorld von den Konkreten Klassen \textit{ABWorld} und ABC textit{ABCWorld.} Diese beiden konkreten Klassen repräsentieren jeweils eine mögliche Welt mit zwei oder drei vorhandenen Variablen. Für jede dieser Variablen existiert ein boolean Wert der true oder false sein kann. Die Welten bieten \textit{toString()} Methoden und Getter für die boolean Variablen. \\
