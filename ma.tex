...
In diesem Abschnitt wird beschrieben, wie die Arbeitsdaten von GenKB verwaltet werden. Während einer Iteration k von GenKB werden dabei die Daten aus der Iteration $ k - 1$ verarbeitet und neue Daten unter $k$ selbst abgelegt. Dabei entstehen große Mengen von Daten und es stellt sich schnell heraus, dass es für den Erfolg des ganzen Programms sehr kritisch ist, diese Daten so effizient und sparsam wie möglich zu verwalten. \\
Die Verwaltung der Daten in kleinen Einheiten übernimmt dabei die Klasse \textit{AbstractPair} mit ihren Implementierungen RealPair und CompressedPair. 
Das Verwalten dieser Objekte übernimmt dann ein PairBuffer, welche unter \autoref{sec:pairbuffer} beschrieben wird.
