...
Als \textit{Paare} wird im Folgenden die Einheit aus einer Wissensbasis und der zugehörigen Menge an Wissensbasen als Kandidaten bezeichnet, um welche die Wissensbasis möglicherweise erweitert werden kann. Im Algorithmus GenKB in \cite{beierle19} werden diese Paare in Zeile 5 initialisiert und in Zeile 8ff bearbeitet. Dargestellt wird diese Einheit als mit charakteristischen dreieckigen Klammern in der Form $<R, C>$, was im Folgenden auch so beibehalten wird. \\
Im Programm werden zwei Implementierungen genutzt, welche hier vorgestellt werden. Beide erben von \textit{AbstractPair}, was die einige Gemeinsamkeiten beider Klassen vereint und ansonsten die Schnittstelle der Paare bildet. Die beiden Implementierungen unterscheiden sich hauptsächlich in der Speicherung der Kandidaten. Da davon sehr viele verwaltet werden müssen, machen sie den allergrößten Teil des Speicherverbrauchs eines Paares aus. Die erste Implementierung ist \textit{RealPair}, hier werden die Kandidaten als Liste von Konditionalen verwaltet. Das ist naheliegend, einfach und schnell, benötigt aber sehr viel Arbeitsspeicher. Die zweite Implementierung \textit{CompressedPair} komprimiert die Liste der Kandidaten und spart dadurch viel Arbeitsspeicher, ist dafür jedoch langsamer. Im späteren Programmablauf verwendet GenKB selbst RealPairs. Auch zum zwischenspeichern der Daten auf der Festplatte werden RealPairs verwendet. Die Klasse CompressedPair wird dazu verwendet, wenn die Daten im Arbeitsspeicher zwischengespeichert werden sollen.
