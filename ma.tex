...
!todo: update. this is never used. describe isToleratedBy() method.
\begin{figure}
\begin{lstlisting}
...
\end{lstlisting}
\caption{\textit{tolerates(AbstractWorld world)} Methode der Klasse PConditional}
\label{code:kb-tolerates}
\end{figure}


Die Toleranz eines Konditionals $B|A$ einer möglichen Welt wird wie in \autoref{def:toleranz}  gezeigt geprüft. Dazu wird in der Methode wie in \autoref{code:kb-tolerates} dargestellt die Toleranz in der Form $\overline{A} \vee B$ in Abhängigkeit der übergebenen möglichen Welt geprüft. 
\\todo:equals besser beschreiben \\
Die \textit{equals(Object o)} Methode ist der Einfachheit halber so implementiert, dass sie true zurückgibt, wenn das übergebene Objekt ein PConditional mit derselben Nummer ist. Das ist wesentlich schneller als die eigentlich korrekte Variante gewesen sein würde. Diese wäre, die AbstractFormulas Antecedent und Consequence jeweils per equals zu vergleichen. Das Ergebnis ist aber in diesem Fall das selbe.
...
!!todo: remove. describe isConsistentWith() instead.
\begin{figure}
\begin{lstlisting}
for (AbstractWorld world : signature.getPossibleWorlds()) {
