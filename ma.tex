...
Wissensbasen werden durch Objekte der Klasse KnowledgeBase repräsentiert. Ein vereinfachtes Klassendiagram ist in \autoref{pic:knowledgebase} zu sehen. Als Attribute besitzt die Klasse einerseits die Nummer der Wissensbasis als int Wert. Zusätzlich besitzt die Klasse eine Liste der enthaltenen Konditionale.  \\
...
Die Datenstruktur, welche die im Abschnitt zuvor beschriebenen Paare aus Wissensbasis und Kandidaten verwaltet, wird im Folgenden als  \textit{PairBuffer} bezeichnet. Der PairBuffer ist damit das Element, welches in GenKb mit der Variable groß $L$ bezeichnet wird. Dabei sind im Programm zwei verschieden Varianten implementiert: Eine Option legt die Daten im Arbeitsspeicher ab, während die andere Option als Speicher die Festplatte nutzt. Beide Varianten besitzen zwei eigene Threads, die der eigentlichen GenKB Implementierung so viel wie möglich Arbeit abnehmen. Der Nutzer kann in der grafischen Oberfläche in den Optionen unter "Buffering" \space wählen, welche der beiden Varianten verwendet werden soll. Der Grund, warum nicht wie von GenKB angedeutet, die Daten in einer einfachen Liste zu verwalten, ist die Menge der entstehenden Daten. Die Liste würde schnell enorme Ausmaße annehmen und innerhalb kurzer Zeit den gesamten Arbeitsspeicher belegen. Die beiden AbstractPairBuffer stellen zwei Strategien dar, um diese Beschränkung durch eine ansonsten viel zu große Liste zu handhaben.
...
todo: newiteration und nextiteration richtig machen \\
...
Daneben noch oberen im Diagramm nicht erwähnte Methoden, um weitere Informationen an die GUI weiterzugeben. Das sind hier Getter für die aktuell belegte Größe der Queues und eine Methode um die Threads außerplanmäßig über den Stop Button zu beenden.

\subsection{KBFileWriter}
Der \textit{KbFileWriter} besitzt zwei gleichermaßen aufgebaute Threads: Ein Thread für die \textit{consistentKbQueue} und ein Thread für die \textit{inconisstentKbQueue}. Die beiden Threads beobachten ihre Queue und sobald sich darin Elemente befinden, werden diese im Ordner für die aktuelle Iteration entweder im Unterordner \glqq consistent \grqq \space oder \glqq inconsistent\grqq \space  abgespeichert. Beide Threads besitzen und aktualisieren zudem jeweils einen Zähler für die Gesamtmenge der Wissensbasis über alle Iterationen und für die aktuelle Iteration.\\
Neben den Threads besitzt der KbFileWriter die Methode \textit{finishIteration()}, die dafür sorgt, dass der aufrufende Thread (der Creator) so lange wartet, bis alle in der Queue vorhandenen Wissensbasen auch korrekt gespeichert und gezählt werden. Die Methode \textit{newIteration(int k)} initialisiert dann die Threads für eine neue Iteration und legt die benötigten Unterordner an.
...
todo: möglicherweise auch am anfang analog zu kb creator? würde die schritte vereinfachen! \\
todo: screenshot \\
Die Grafische Oberfläche des KBCreators ist in XXX gezeigt und besteht aus zwei Teilen: Die linke Seite beinhaltet die möglichen Optionen, die zum generieren der Wissensbasis eingestellt werden können. Einige Optionen sind dabei von anderen Abhängig und werde ausgegraut dargestellt, wenn sie in der aktuellen Einstellung nicht sinnvoll sind. Am unteren Ende der Einstellungen befinden sich der Start und der Stop Button für den Creator. \\
Auf der Rechten Seite befindet sich die Anzeige für den aktuellen Status mit vielen Details. Wenn der Creator nicht läuft, ist die gesamte Rechte Seite ausgegraut. Wenn er läuft, werden die Informationen schwarz dargestellt und laufend aktualisiert, die Aktualisierung übernimmt ein eigener Thread. Wenn der Creator gestoppt wird, wird die rechte Seite wieder ausgegraut. Die Informationen vom Zeitpunkt des Beenden des Creators werden jedoch weiterhin angezeigt.
