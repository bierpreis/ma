...
%todo: other style which supports url
...
\label{sec:äquivalenz-konditionale} %todo: this äquivalenz != äquivalenz durch umbenennung!! nochmal neu überlegen. alles richtig übersetzt?
...
Eine sehr wichtige Methode der Klasse WConditonal ist die Überschreibung der \textit{equals(Object o)} Methode. Diese ist in \autoref{code:equals-wconditonal} zu sehen. Sie liefert dann true zurück, wenn die WorldsList Objekte beider WConditonals dieselben sind.
...
%todo: eventuell das hier verlinken bei beschreibung wconditonal?
%todo: nochmal lesen und korrigieren
\subsection{Counter Conditionals}
Für den Algorithmus GenKB ist es nötig, dass zu einem Konditional schnell dass Counter Konditional gefunden werden kann. Mit der Klasse WConditional ist das recht einfach möglich. Analog zum Finden der äquivalenten Konditionale besitzt dazu jedes Konditional zum einen eine Methode, um zunächst ein \textit{neues} Counter Conditional zu erstellen. Wenn dann alle Konditionale erstellt sind, wird nach dem tatsächlichen Objekt des Counter Conditional gesucht und dieses im ursprünglichen Konditional gesetzt. \\
Die Methode \textit{getBasicCounterConditional()} um das vorläufige Counter Conditonal zu generieren ist unter   \autoref{code:basic-counter} zu sehen. Als Antecedent für das neue Konditonal wird das Antecedent des alten Verwendet, dieses bleibt ja gleich. Als Consequence wird die Liste der Welten des Antecedent minus der Liste der Welten der Consequence  des ursprünglichen Konditionals gebildet. Die Methode \textit{getBasicCounterConditional()} wird dann später verwendet, um mit der Methode \textit{findCounterConditional(...)} aus \autoref{code:real-counter} über NFC zu iterieren und das tatäschliche Counter Conditonal zu setzen.
