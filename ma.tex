...
In diesem Abschnitt wird gezeigt, wie der konkrete Algorithmus GenKb aus \cite{beierle19} zur Generierung der Wissensbasen implementiert wurde. Wie schon erwähnt, befindet sich der Algorithmus in eigenem Objekt der Klasse GenKb, ein stark vereinfachtes Klassendiagramm dieses Objekts befindet sich in \autoref{pic:genkb}. Dieses Objekt implementiert das Java Interface \textit{Runnable} und kann somit als eigener Thread agieren. Als Referenzen im Objekt befinden sich die Listen von PConditonals cnfc und nfc, die vom NfcCreator erstellt wurden und bereits erklärt wurden. Dann befindet sich eine Referenz auf das Objekt \glqq l \grqq \space, was dem \glqq l \grqq \space aus der Originaldarstellung von GenKb entspricht. Hauptsächlich ist es ein Objekt, was eine \textit{List<List<AbstractPair>>} beinhaltet, also die Paare aus \textit{<Konditional, Kandidaten>} während der Iterationen die Daten aufnimmt. Das Objekt \textit{kbWriter} kümmert sich um das schreiben der erstellten Wissensbasen. Die Klassen AbstractPairBuffer und AbstractPairWriter werden in den nächsten Abschnitten beschrieben. \\
In \autoref{code:genkb-gesamt} und \autoref{code:genkb:init} ist nun die Impelementierung von GenKb dargestellt. Der Algorithmus ist auf zwei Methoden verteilt damit keine einzelne übermäßig lange Methode erstellt. Die Aufteilung ist wie folgt: Der Eigentliche Algorithmus ist in \autoref{code:genkb-gesamt}, dieser ruft dann für Zeile 3-5 von GenKb die andere Methode zum initialisieren der Wissensbasen mit  jeweils einem Element auf. \\
...
\end{lstlisting}
\label{code:genkb:init}
\caption{Initialisierung der 1-elementigen Wissensbasen in GenKb, entspricht Zeile 3-5}
\end{figure}
