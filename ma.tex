...
\def\QEQ{{%
    \setbox0\hbox{<}%
    \rlap{\hbox to \wd0{\hss \raisebox {0.4pt}{$\ \cdot$\hss}}}\box0
}}
...
Ein Konditional ist die Verbindung aus einer Vorbedingung und einer Folgerung aus dieser Vorbedingung. Ein qualitatives Konditional ist ein solches unter  Unsicherheit und entspricht einem Zusammenhang in der Form \glqq Wenn $A$, dann normalerweise $B$\grqq . Dargestellt wird so ein Konditional  im folgenden mit dem Symbol \glqq$|$\grqq \space in der Form $r = ( \lag | \lag)$. Ein Beispiel ist $r = (B|A)$, das dazugehörige gegenteilige Konditional lautet dann $\overline{r} = (\overline{B}|A)$.\\
Um die Akzeptanz eines unsicheren Konditionals zu bestimmen, werden sie oft im Rahmen von ordinalen Rangfunktionen nach Wolfgang Spohn betrachtet, wie beispielsweise in \cite{spohn88} oder \cite{spohn12} beschrieben. Mehr dazu auch in diesem Text im Abschnitt zu  \hyperref[sec:Wissensbasen]{Wissensbasen}. Grundsätzlich wird ein unsicheres Konditional dann  akzeptiert, wenn es plausibler ist als das gegenteilige Konditional. Beispielsweise $(B|A)$ wird  dann akzeptiert, wenn $A \wedge B$ plausibler ist als $A \wedge \overline{B}$, siehe dazu auch \cite{isberner14}. \\
Das Konditional unter Unsicherheit in der Form $(B|A)$ teilt dann die Menge möglicher Welten $\Omega$ in drei Teile auf: Welten, die das Konditional bestätigen, Welten die es falsifizieren und Welten, auf die das Konditional nicht anwendbar ist, weil die Vorbedingung nicht erfüllt ist. Die letztere Möglichkeit wird mit $u$ bezeichnet, was als \textit{unknown} gedeutet werden kann. Damit lässt sich das Konditional als Indikatorfunktion wie folgt darstellen:
\[
...
\end{equation}
Als triviale Konditionale werden solche beschrieben, die keinen Mehrwert bieten, vgl. \cite{beierle19}. Das beinhaltet sowohl sich selbst erfüllende in der Form $(A \models A)$ als auch Konditionale, die ein anderes einfach das Gegenteil eines anderen Konditionals darstellen, wie beispielsweise $(A \models \overline{B})$. Solche Konditionale sind bei der weiteren Betrachtung nicht von Interesse und werden daher nicht weiter beachtet.
...
Um diese Notation etwas kompakter darzustellen, wird eine Darstellung $[[\omega]]_{\dotl}$ für eine mögliche Welt unter der Signatur $\Sigma_{abc}$ per Zahl definiert, angelehnt an deren Interpretation als Binärzahl: \\
$[[abc]]_{\dotl}  = 7,\ 
...
\item{Komplettheit: für jedes nichttriviale Konditional in $\Sigma$ existiert ein äquivalentes Konditional in $NFC$ }
...
Durch Umbenennung von Variablen ist es möglich, neue Kombinationen von Welten, Konditionalen und Wissensbasen zu schaffen, die sich außer der Umbenennung nicht von den Vorhandenen unterscheiden. Dieses Verhalten wird als Isomorphismus beschrieben. Die Menge an durch diesen Isomorphismus entstandenen Dingen wird dann als Äquivalenzklasse bezeichnet. Beispielsweise enthält die Signatur $\Sigma_{abc}$ die mögliche Welt $\overline{a}bc$. Wenn jetzt $a$ in $b$ und $b$ in $a$ umbenannt wird, entsteht der Isomorphismus $a\overline{b}c$. Analog kann durch Umbenennung daraus $ab\overline{c}$ entstehen. Alle drei dieser möglichen Welten bilden zusammen eine Äquivalenzklasse. Entsprechend ergibt sich die folgende Definition.
\begin{theorem}[Isomorphismus](entnommen aus \cite{beierle19}) \ \\
$X, X'$ sind Welten, Aussagen, Wissensbasen oder Mengen dieser Dinge. $X$ und $X'$ sind dann isomorph $(X \simeq X')$, wenn eine Umbenennung existiert in der Form  $\rho(X) = X'$.
\end{theorem}
Für die Signatur $\Sigma_{abc}$ ergeben sich dann die folgenden Äquivalenzklassen: \\
$[abc]$, $[ab\overline{c}, a\overline{b}c\, \overline{a}bc], [a\overline{b}\overline{c}, \overline{a}b\overline{c}, \overline{a}\overline{b}c], [\overline{a}\overline{b}\overline{c}]$ \\
Die Menge der Äquivalenzklassen in einer Menge $M$ mit dem Element $m \in M$ wird im Folgenden mit $[M]_{\setminus\simeq}$ und die konkrete Äquivalenzklasse welche $m$ enthält mit $[m]_\equiv$ notiert.
...
\begin{theorem}[$cNFC$ $und$ $Ordnungsrelation$ $\rdotl$](entnommen aus \cite{beierle19}) \ \\
Gegeben sei eine Signatur $\Sigma$ mit linearer Ordnung $\dotl$. Dann sind $[NFC(\Sigma)]_\simeq=\{[r_1]_\simeq,...,[r_m]_\simeq\}$ die Äquivalenzklassen von $NFC(\Sigma)$ induziert durch Isomorphismus sodass für jedes $i \in \{1, ..., m\}$ das Konditional $r_i$ das minimale Element in $[r_i]_\simeq$ im Sinne von $\dotl$ ist und $r_1\dotl...\dotl r_m$ ist.
\begin{itemize}
...
!!! wie viele Konditionale wären theoretisch möglich?  siehe in \cite{beierle19} auf seite 3 mitte. wie sieht das für abc aus???
