...
%\\todo:re read \\
Der Algorithmus, der alle Teilmengen aus den enthaltenen Welten einer Signatur berechnet, ist in \autoref{code:subsets} dargestellt. Als Input wird die Liste möglicher Welten der Signatur verwendet, der Output ist eine Liste aller Teilmengen des Inputs ohne der leeren Menge. Im Algorithmus wird zunächst in Zeile 3 über alle Integers der möglichen Welten iteriert. In Zeilen 10-12 werden die \textit{WorldsLists} mit jeweils einem Element erstellt. In Zeile 4 wird dann über die schon bestehenden \textit{WorldsLists} iteriert und jeweils alle Kombinationen mit der aktuellen ein-elementigen Welt als neue \textit{WorldsLists} erstellt. Am Ende enthält dann die \textit{List<WorldsList> subsetList} WorldsLists mit allen möglichen Teilmengen der ursprünglich übergebenen Liste von Integers. \\
